        ...
        <el-row :gutter="8" style="margin-top:20px;" v-if="!noList">
            <el-col :span="6" v-for="(item,index) in tableData" :key="item.id" >
                <div class="show_check_box">
                    ...
                    <div class="grid_content" @click="checkEdit(item.id,index,$event)" v-on:mouseenter="checkLength(item.content,index)" v-on:mouseleave="isShowAll = -1">
                        <div class="show_content" v-html="item.contentFilter">
                        </div>
                        <span class="show_all" v-show="isShowAll == index" v-html="item.contentFilter">
                        </span>
                    </div>
                </div>
            </el-col>
...
export default {
    data() {
        return {
            baseUrl: BASE_URL,
            api: {
                list: "/commentAudit/queryCommentAuditList",
                submit:"/commentAudit/doCommentAudit",
                exit:"/commentAudit/exitCommentAudit",
            },
            editId:"", //操作id
            type :"", //审核类型
        ...
        getTableData() {
                ...
                setTimeout(() => {
                    ...
                    this.$http.get(this.api[this.listType], param, (data, res) => {
                        let dataList = data.data.comments;
