            ...
            <el-col :span="6" v-for="(item,index) in tableData" :key="item.id" >
                <div class="show_check_box">
                    <div>
                        ...
                        <div class="user_name" @click="goUserDetail(item.userId,item.userType)">{{item.userName}}</div>
                        <div class="user_message">
                            <span class="show_report_name">举报人：{{item.reportUserName}}</span>
                            <span class="show_report_id">举报人ID：{{item.reportUserId}}</span>
                            <span class="show_report_id">举报次数：{{item.times}}</span>
                            <span class="show_report_id">
                                <span v-if="item.reason == 1">举报原因：广告信息</span>
                                <span v-if="item.reason == 2">举报原因：黄色暴力</span>
                                <span v-if="item.reason == 3">举报原因：政治敏感</span>
                                <span v-if="item.reason == 4">举报原因：辱骂他人</span>
                                <span v-if="item.reason == 5">举报原因：其他原因</span>
                            </span>
                            <div class="comment_message">
                                <span>{{item.createTime}}</span>
                                <!-- <span class="show_artInfo" @click="goArtDetail(item.id)" >{{item.id}}</span> -->
                                <span class="show_artInfo">ID:{{item.targetId}}</span>
                            </div>
                        </div>
                    </div>
                    <div class="grid_content" @click="checkEdit(item.targetId,index,$event)" v-on:mouseenter="checkLength(item.content,index)" v-on:mouseleave="isShowAll = -1">
                        <div class="show_content" v-html="item.content">
                        </div>
                        <span class="show_all" v-show="isShowAll == index" v-html="item.content">
                        </span>
                    </div>
                </div>
            </el-col>
...
export default {
    data() {
        return {
            baseUrl: BASE_URL,
            api: {
                list: "/report/commentAuditList",
                submit:"/report/doReportCommentAudit",
                exit:"/report/exitReportCommentAudit",
            },
            editId:"", //操作id
            type :"", //审核类型
                    ...
                    this.$http.post({
                        ...
                        success: data => {
                            ...
                            for(let j = 0,len = dataList.length; j < len; j++) {
                                let commId = dataList[j].targetId;
            ...
            submitAudit(){  
                ...
                if(this.dataArr.length > 0 || this.rejectArr.length > 0){
                    setTimeout(() => {
                        ...
                        param = {
                                auditNo: this.editId,
                                passCommentIds: this.dataArr,
                                rejectCommentIds: this.rejectArr,
                            };
                        this.$http.post({
                            isJson: true,
                            url: this.api["submit"],
