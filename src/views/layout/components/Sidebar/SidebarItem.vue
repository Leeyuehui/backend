<template>
  <div class="menu-wrapper">
    <template v-for="item in routes">
      ...
      <!-- <router-link v-if="!item.hidden&&item.children&&item.children.length===1" :to="item.children[0].path" :key="item.children[0].name">
     ...
     <el-submenu v-if="!item.hidden&&item.children&&item.children.length>0" :index="item.name||item.path" :key="item.name">
       <template slot="title">
