...
<div class="app-container">
    <el-form :inline="true"  class="demo-form-inline"> <!--  :model="formInline" -->
        <el-form-item label="">
            <el-input v-model="id" placeholder="请输入话题ID" style="width:150px;"></el-input>
        </el-form-item>
        <el-form-item label="">
        ...
        <el-table v-loading="loading" element-loading-text="正在加载中" :data="tableData" :border="true" v-if="tableData" style="width: 100%">
            <el-table-column prop="targetId" label="话题ID" align="center"></el-table-column>
            <el-table-column prop="targetName" label="话题名称" align="center"></el-table-column>
            <el-table-column prop="postCount" label="发帖量" align="center">
                <template slot-scope="scope">
                    <router-link :to="{name:'postData',query:{topicId:scope.row.targetId}}"><div style="color:#87C3E6;">{{scope.row.postCount}}</div></router-link>
                </template>
            </el-table-column>
...
export default {
    ...
    methods:{
        ...
        // 导出
        exportList(){
            window.location.href = baseUrl+'/stat/exportTopic?pageSize=20&startPage=1&id='+this.id; 
        },
